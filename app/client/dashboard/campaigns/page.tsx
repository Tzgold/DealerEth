  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href="/client/dashboard/post" className="rounded-full bg-gradient-to-r from-[#FE2C55] via-[#ff5f8a] to-[#25F4EE] px-4 py-2 text-xs font-bold text-white">
          Post new campaign
        </Link>
      </div>
            ...
            <li key={campaign.id} className="rounded-2xl border border-white/10 bg-[#141416] p-5">
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <span className="rounded-full border border-[#25F4EE]/40 bg-[#25F4EE]/15 px-3 py-1 text-[11px] font-bold uppercase text-[#25F4EE]">
                  Live · {campaign.applications.length} apps
                </span>
              </div>
              ...
              <Link href={`/client/dashboard/messages?campaign=${campaign.id}`} className="mt-4 inline-block text-xs font-semibold text-[#25F4EE] underline">
                View applications
              </Link>
            </li>
          ))
